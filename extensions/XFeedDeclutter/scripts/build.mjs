...
const EXTENSION_NAME = IS_PROD ? 'X Feed Declutter & Focus' : 'X Feed Declutter & Focus (Dev)';

// Chrome match patterns support only a single wildcard, at the very start of
// the hostname — so both domains X currently resolves on are enumerated
// explicitly rather than attempting a shorthand pattern (PRD §6).
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Forces X back to a plain, chronological, ad-free feed: default to Following, hide ads, hide algorithmic sidebar modules, optionally hide vanity counts, and a focus-mode reading layout. No account, no network requests.',
    icons: {
    ...
    action: {
      default_title: 'X Feed Declutter & Focus',
      default_popup: 'popup.html',
    },
    // Exactly the surface named in PRD-24 §6: this only reads/hides what's
    // already rendered in the current tab and remembers five toggles plus
    // two local counters. No activeTab, no downloads — there is nothing to
    // export (PRD §6/§10).
    permissions: ['storage'],
