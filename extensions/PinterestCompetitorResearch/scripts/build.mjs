...
const EXTENSION_NAME = IS_PROD ? 'Pinterest Competitor Pin Research' : 'Pinterest Competitor Research (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Save competitor pins to a local research library — title, full description, destination and your notes — then export as CSV, Markdown or JSON. No account, no cloud.',
    icons: {
    ...
    action: {
      default_title: 'Open competitor research panel',
    },
    // Pinterest's regional ccTLDs (pinterest.de, pinterest.co.uk, …) all
    // 308-redirect to a locale subdomain of pinterest.com (e.g. de.pinterest.com),
    // which this single pattern already covers — see PRIVACY.md.
