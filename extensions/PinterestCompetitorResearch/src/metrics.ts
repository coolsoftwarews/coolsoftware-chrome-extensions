 ...
 *
 * PRD §9 wants habit numbers (saves/week, users who export), and the hard
 * constraint in docs/extensions/README.md is that instrumentation
 * never leaves the device. Every event here is a counter in
 * chrome.storage.local: no pin URLs, no titles, no descriptions, no
 * timestamps beyond a day bucket. The user can read and reset them from the
 * panel's Usage sheet.
 *
 * The thumbnail capture rate is the health number that matters most — if
 * capture starts failing and falling back to remote URLs, the library is
 * quietly rotting (PRD §6), same convention as the sibling savers.
 */

const STORAGE_KEY = 'pcr:metrics';
