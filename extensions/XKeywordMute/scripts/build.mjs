...
const EXTENSION_NAME = IS_PROD ? 'X Keyword Mute & Filter List' : 'Keyword Mute (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      "Mutes keywords and phrases in your X timeline — regex for power users, bundled starter filter packs, and a hit count per rule. No account, no cloud, no automation.",
    icons: {
    ...
    action: {
      default_title: 'X Keyword Mute',
      default_popup: 'popup.html',
    },
    // Minimum permissions (PRD §6): storage for the local rule list, and
    // host access to the two X hosts the content script runs on. No
    // `activeTab`, no `downloads`, no `sidePanel` — the popup opens itself
    // and exports go through a plain <a download> anchor instead of the
