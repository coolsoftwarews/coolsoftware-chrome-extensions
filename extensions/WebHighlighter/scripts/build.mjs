...
const EXTENSION_NAME = IS_PROD ? 'Web Highlighter & Markdown Export' : 'Web Highlighter (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Highlight any page, add notes, export as Markdown, HTML, PDF or TXT. No account, no cloud — everything stays on your device.',
    icons: {
    ...
    action: {
      default_title: 'Open highlights panel',
    },
    // <all_urls> is unavoidable for a universal highlighter (PRD §6). It buys
    // nothing but the ability to restore marks — there is no network code here.
