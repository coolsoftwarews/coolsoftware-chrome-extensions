...
const EXTENSION_NAME = IS_PROD ? 'LinkedIn Job Application Tracker' : 'LinkedIn Job Tracker (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Track LinkedIn job postings through a local kanban — Saved, Applied, Interviewing, Offer, Rejected, Withdrawn — with notes and CSV/Markdown/JSON export. No account, no cloud, read-only on LinkedIn.',
    icons: {
    ...
    // Minimum permissions (PRD §6): activeTab + storage + downloads to run the
    // track flow and export files, sidePanel because the tracker is a side
    // panel. Host access is scoped to LinkedIn only — this extension has no
    // reason to run anywhere else, and does not ask to.
