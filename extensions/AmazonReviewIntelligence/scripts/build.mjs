...
const EXTENSION_NAME = IS_PROD ? 'Amazon Review Intelligence' : 'Amazon Review Intelligence (Dev)';

/**
 * Chrome match patterns don't support a wildcard TLD ("amazon.star" is not
 * valid), so the PRD's shorthand host pattern (PRD §6) is expanded here into
 * the Amazon storefronts the product is meant to work on — the three the PRD
 * names explicitly for the spike (.com/.co.uk/.de) plus the other major
 * locales. Nothing broader: no all-sites wildcard, no other retailer.
 */
const AMAZON_TLDS = [
  'com', 'co.uk', 'de', 'fr', 'it', 'es', 'nl', 'se', 'pl',
  'ca', 'com.mx', 'com.br', 'co.jp', 'in', 'com.au', 'ae', 'sa', 'sg',
];
const HOST_PERMISSIONS = AMAZON_TLDS.map(tld => `*://*.amazon.${tld}/*`);
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Cluster the recurring complaints and praise in an Amazon product\'s reviews. No account, no cloud — everything stays on your device.',
    icons: {
    ...
    action: {
      default_title: 'Amazon Review Intelligence',
      default_popup: 'panel.html',
    },
    // Minimum permission set per PRD §6: activeTab, storage, downloads, plus
    // host access limited to Amazon storefronts (never <all_urls>).
