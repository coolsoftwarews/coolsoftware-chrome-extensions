 ...
 * Instagram page — this popup never has to talk to the content script.
 */
...
const LABELS: Record<string, string> = {
  profile_analysed: 'Profiles analysed',
...
const els = {
  gate: $('gate'),
  intro: $('intro'),
  openInstagram: $<HTMLButtonElement>('open-instagram'),
  metrics: $('metrics'),
...
async function renderMetrics(): Promise<void> {
  ...
  // The health metric from PRD §9 — always shown, even at zero.
...
async function checkPage(): Promise<void> {
  ...
  const onInstagram = /^https?:\/\/([^/]+\.)?instagram\.com\//.test(tab?.url ?? '');
  els.gate.hidden = onInstagram;
  els.intro.hidden = !onInstagram;
}

els.openInstagram.addEventListener('click', () => {
  void chrome.tabs.create({ url: 'https://www.instagram.com/' });
});
...
els.exportData.addEventListener('click', async () => {
  ...
  try {
    await chrome.downloads.download({
      ...
      filename: `instagram-outlier-finder-backup-${stamp}.json`,
      saveAs: false,
    });
    setStatus(`Exported ${backup.profiles.length} cached profile${backup.profiles.length === 1 ? '' : 's'}.`);
  } catch {
...
els.importFile.addEventListener('change', async () => {
  ...
  try {
    ...
    setStatus(`Imported ${result.profiles} cached profile${result.profiles === 1 ? '' : 's'}.`);
  } catch (error) {
