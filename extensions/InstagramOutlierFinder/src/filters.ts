 ...
 * hiding posts because a count failed to parse would make the grid look
...
export function matchesFilter(post: ScoredPost, filter: FilterState, now = Date.now()): boolean {
  ...
  if (filter.kind === 'reels' && post.kind !== 'reel') return false;
  if (filter.kind === 'posts' && post.kind === 'reel') return false;

  if (filter.days !== null && post.takenAt !== null) {
    if (post.takenAt < now - filter.days * DAY_MS) return false;
  }
...
/** The filtered, sorted set the header count, badges dimming and export all read from. */
