/**
 * Local-only usage counters (PRD §9): profiles analysed, filter used, sort
 * used, export used, and parse-failure count. No handles, no URLs, no post
 * ids — only totals and the dates the extension was used, readable and
 * resettable from the popup.
 *
 * The parse-failure counter is the health metric PRD §9 calls out by name:
 * Instagram will change its DOM, and this is the only place that shows it
 * before the reviews do.
 */

import { MetricEvent, Metrics } from './types';

const STORAGE_KEY = 'iof:metrics';
