...
const EXTENSION_NAME = IS_PROD ? 'Instagram Outlier Finder' : 'Instagram Outlier Finder (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Badges Instagram profile grid posts with their outlier ratio against the account’s own recent median. No account, no backend, no network requests.',
    icons: {
    ...
    action: {
      default_title: 'Instagram Outlier Finder',
      default_popup: 'popup.html',
    },
    // PRD §6: activeTab, storage, downloads + the instagram.com host. Nothing broader.
