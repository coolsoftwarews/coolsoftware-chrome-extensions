...
const EXTENSION_NAME = IS_PROD ? 'YouTube Focus Timer' : 'YouTube Focus Timer (Dev)';

// PRD-32 §6: host access is YouTube only, nothing broader.
const YOUTUBE_MATCH_PATTERNS = ['*://*.youtube.com/*'];
...
async function writeManifest(backgroundScript, contentScript, contentStylesheet, popupScript) {
  const manifest = {
    manifest_version: 3,
    name: EXTENSION_NAME,
    version: EXTENSION_VERSION,
    description:
      'Hide YouTube recommendations, Shorts, end screens and comments, and see an honest local report of your time on YouTube today and this week. No account, no cloud.',
    icons: {
    ...
    action: {
      ...
      default_title: 'YouTube Focus Timer',
    },
    // Minimum permission set (PRD-32 §6): storage for settings/report,
    // downloads for the CSV/JSON exports, and host access limited to
    // YouTube itself — nothing broader.
    permissions: ['storage', 'downloads'],
    host_permissions: YOUTUBE_MATCH_PATTERNS,
    background: {
  ...
  await fs.promises.writeFile(path.join(distDir, 'manifest.json'), JSON.stringify(manifest, null, 2), 'utf8');
}
...
function buildOptions() {
  ...
  return {
    background: {
      ...
      format: 'iife', // service workers cannot be modules without manifest "type": "module"; iife keeps this simple
      entryNames: '[name]',
    },
    content: {
      ...shared,
...
async function finalize(metas) {
  ...
  await writeManifest(backgroundScript, contentScript, 'content.css', popupScript);
}
...
async function buildOnce() {
  ...
  const [background, content, popup] = await Promise.all([build(opts.background), build(opts.content), build(opts.popup)]);
  await finalize({
