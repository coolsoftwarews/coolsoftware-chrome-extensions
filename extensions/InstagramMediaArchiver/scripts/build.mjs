...
const EXTENSION_NAME = IS_PROD ? 'Instagram Media Archiver' : 'Instagram Media Archiver (Dev)';
...
// PRD §6's non-functional requirements table names the permission set
// exactly: activeTab, downloads, storage + host access to instagram.com.
// Nothing broader — no <all_urls>, no "tabs", no "scripting". The content
// script matches Instagram's own post/feed pages; host_permissions is
// scoped identically, since this extension has no reason to run anywhere
// else and does not ask to (PRD §5: the ownership gate is this product's
// entire premise, and it only makes sense on instagram.com).
const HOST_PATTERNS = ['*://*.instagram.com/*'];

async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    manifest_version: 3,
    name: EXTENSION_NAME,
    short_name: 'IG Archiver',
    version: EXTENSION_VERSION,
    description:
      "Save the photos and videos from your own Instagram posts to your device in one click. Never offers Save on a post you didn't author.",
    icons: {
    ...
    action: {
      default_title: 'Instagram Media Archiver — saved items',
