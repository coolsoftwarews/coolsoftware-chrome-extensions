...
const EXTENSION_NAME = IS_PROD ? 'YouTube Sign-in-Free Playlist Sorter' : 'Playlist Sorter (Dev)';
...
async function writePanelHtml(panelScriptName) {
  const template = await fs.promises.readFile(path.join(srcDir, 'panel', 'panel.html'), 'utf8');
...
async function copyStaticAssets() {
  await fs.promises.copyFile(path.join(srcDir, 'panel', 'panel.css'), path.join(distDir, 'panel.css'));
}

// Chrome match patterns support only one wildcard, at the very start of the
// hostname (*.example.com) — a PRD's shorthand like *://*.youtube.*/* is not
// valid manifest syntax. YouTube has one primary consumer domain, so this is
// simple, but keep it a named list (not an inline literal) so it stays the
// single place this is decided.
const HOST_PERMISSIONS = ['*://*.youtube.com/*'];
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      "Sort any YouTube playlist by duration, title or date from a side panel — yours or anyone else's. No sign-in, no API key, nothing leaves your device.",
    icons: {
    ...
    action: {
      default_title: 'Playlist Sorter',
    },
    // Exactly the PRD §6 list — no scripting, no tabs, no broader host access.
...
function buildOptions() {
  ...
  return {
    ...
    panel: {
      ...shared,
      entryPoints: [path.join(srcDir, 'panel', 'panel.ts')],
      format: 'esm',
      entryNames: 'panel',
    },
  };
...
async function finalize(metas) {
  ...
  const contentScript = outputNameFor(metas.content, path.join(srcDir, 'content', 'index.ts'));
  const panelScript = outputNameFor(metas.panel, path.join(srcDir, 'panel', 'panel.ts'));
