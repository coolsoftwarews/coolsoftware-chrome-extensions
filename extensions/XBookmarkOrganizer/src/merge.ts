 ...
 *
 * Items and collections are matched by id: the incoming file wins on
 * conflicts, the same rule WebHighlighter and XConversationSaver use for
 * their imports. That makes "import the same backup twice" idempotent, and
 * makes restoring an older backup predictable (it's the imported file's data
 * for any id it names — ids it doesn't name are left alone). This is "merge,
 * not overwrite" at the store level (PRD §4): nothing already on the device
 * is dropped, only ids present in the file are touched.
 */

import { Backup, BookmarkItem, Collection } from './types';
...
export function mergeBackup(local: LocalData, incoming: Partial<Backup>): MergeResult {
  ...
  for (const item of incomingItems) {
    if (!item?.id || !item.post?.id) continue;
    validItems++;
