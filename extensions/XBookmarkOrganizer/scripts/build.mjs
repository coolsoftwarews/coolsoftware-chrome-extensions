...
const EXTENSION_NAME = IS_PROD ? 'X Bookmark Organizer' : 'X Bookmark Organizer (Dev)';
...
// PRD-26's non-functional requirements table names the permission set
// exactly: activeTab, storage, downloads, sidePanel + host access to x.com
// and twitter.com. Nothing broader: no <all_urls>, no "tabs", no
// "scripting". host_permissions cover the whole site (the panel's
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Organize and search your own X bookmarks locally — folders, tags, notes, and export to Markdown, CSV or JSON. No account, no cloud, no sync with X.',
    icons: {
    ...
    action: {
      default_title: 'Open X Bookmark Organizer',
    },
    ...
    commands: {
      'open-library-panel': {
        ...
        description: 'Open the X Bookmark Organizer library panel',
      },
    },
