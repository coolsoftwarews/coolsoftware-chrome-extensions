...
const EXTENSION_NAME = IS_PROD ? 'LinkedIn Feed Focus & Algorithm Control' : 'LinkedIn Feed Focus (Dev)';
...
async function writeManifest(backgroundScript, contentScript, popupScript) {
  const manifest = {
    ...
    description:
      'Hide promoted posts, "People you may know" modules, trending news, suggested posts and reaction counts on your LinkedIn feed — one popup, a handful of toggles. No account, no cloud, read-only.',
    icons: {
    ...
    action: {
      ...
      default_title: 'LinkedIn Feed Focus',
    },
    // Minimum permission set (PRD §6): storage only, and host access limited
    // to LinkedIn itself — no activeTab, no downloads (there is nothing to
    // export), no sidePanel, no scripting/tabs.
    permissions: ['storage'],
  ...
  // popupScript is folded into popup.html by writePopupHtml, not referenced
  // directly by the manifest — kept as a parameter so callers can't forget
  // to build/copy it before the manifest is written.
  void popupScript;
