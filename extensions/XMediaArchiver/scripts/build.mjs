...
const EXTENSION_NAME = IS_PROD ? 'X Media Archiver' : 'X Media Archiver (Dev)';
// Chrome caps short_name at 12 characters.
const SHORT_NAME = 'X Archiver';

// Chrome match patterns only support a single wildcard, at the very start of
// the hostname — X's two live hostnames are enumerated explicitly rather
// than any *.x.* shorthand (portfolio build memory).
...
// Only what PRD-46 §6 lists: activeTab/downloads/storage + the two X
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Save the photos and videos from your own X posts to your device. Own content only, enforced automatically.',
    icons: {
    ...
    action: {
      ...
      default_title: 'X Media Archiver',
    },
