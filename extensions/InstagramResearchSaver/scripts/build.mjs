...
const EXTENSION_NAME = IS_PROD ? 'Instagram Creator Research Saver' : 'Instagram Research Saver (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Save Instagram posts and Reels to a local research library with collections, notes and numbers. Export as CSV, Markdown or JSON. No account, no cloud.',
    icons: {
    ...
    action: {
      default_title: 'Open research library',
    },
    // Minimum permissions (PRD §7): activeTab + storage + downloads to run the
    // save flow and export files, sidePanel because the library is a side
    // panel. Host access is scoped to Instagram only — this extension has no
