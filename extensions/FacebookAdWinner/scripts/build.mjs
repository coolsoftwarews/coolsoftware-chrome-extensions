...
const EXTENSION_NAME = IS_PROD ? 'Meta Ad Winner' : 'Meta Ad Winner (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Surfaces the ads in Meta’s Ad Library that have run longest and in the most variants, with filters, sort and a swipe file. No account, no cloud.',
    icons: {
    ...
    action: {
      default_title: 'Meta Ad Winner — swipe file',
      default_popup: 'panel.html',
    },
    // Exactly the permission set the PRD specifies (PRD-14 §6) — nothing
    // broader. No `sidePanel`, `scripting` or `tabs`: the content script is
    // declaratively injected by the manifest, and `activeTab` plus the
    // popup's own `chrome.tabs` access cover everything else.
    ...
    content_scripts: [
      {
        // Scoped to the Ad Library itself, not all of facebook.com — the
        // host permission has to cover the whole domain (Meta serves the
        // Library from plain facebook.com), but the content script has no
        // business running anywhere else on the site.
        matches: ['*://*.facebook.com/ads/library/*', '*://*.facebook.com/ads/library'],
        js: [contentScript],
