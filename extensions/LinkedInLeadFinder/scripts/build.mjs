...
const EXTENSION_NAME = IS_PROD ? 'LinkedIn Engagement Lead Finder' : 'LinkedIn Lead Finder (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Collect the people commenting on a LinkedIn post, qualify them with your own keyword rules, and export a CSV or Markdown lead list. Read-only, no account, no cloud.',
    icons: {
    ...
    action: {
      default_title: 'Open lead panel',
    },
    // Minimum permission set the PRD calls for (PRD §6): activeTab, storage,
    // downloads, plus sidePanel for the lead list UI itself. No "scripting" —
    // the content script is declared statically below and needs nothing more.
