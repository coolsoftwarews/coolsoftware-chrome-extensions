...
const EXTENSION_NAME = IS_PROD ? 'LinkedIn Post Outlier Finder' : 'LinkedIn Post Outlier Finder (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Badges any LinkedIn profile post history, hashtag page or content search result with each post’s outlier ratio against that specific author’s own recent median. No account, no backend, no network requests.',
    icons: {
    ...
    action: {
      default_title: 'LinkedIn Post Outlier Finder',
      default_popup: 'popup.html',
    },
    // PRD §6: activeTab, storage, downloads + the linkedin.com host. Nothing broader — no sidePanel.
