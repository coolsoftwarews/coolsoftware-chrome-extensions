 ...
 *
 * The PRD flags PDF generation as the main bundle-size risk (a full renderer or
 * an embedded font would blow the 500 KB budget on its own). This produces a
 * plain PDF 1.4 with the two standard Helvetica faces, which every viewer has
 * built in, so the whole export path costs a few kilobytes.
 *
