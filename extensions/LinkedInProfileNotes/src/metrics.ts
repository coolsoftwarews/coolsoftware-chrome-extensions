 ...
 * with ≥5 notes) need instrumentation, but there is no backend and no
 * identity. Every event is a counter in chrome.storage.local; nothing here
 * carries a person's name, headline, or note text. The user can read or
 * clear these from the panel footer.
 */

const STORAGE_KEY = 'lpn_metrics';

export type MetricEvent =
  | 'panel_opened'
  | 'note_saved'
  | 'note_deleted'
  | 'fuzzy_match_suggested'
