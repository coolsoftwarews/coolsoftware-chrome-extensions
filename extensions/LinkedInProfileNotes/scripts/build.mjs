...
const EXTENSION_NAME = IS_PROD ? 'LinkedIn Profile Notes' : 'LinkedIn Profile Notes (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'A private note on any LinkedIn profile you visit — free text plus an optional tag, keyed by the profile itself. No account, no CRM, no cloud, read-only.',
    icons: {
    ...
    action: {
      default_title: 'Open your noted profiles',
    },
    // Scoped to LinkedIn only (PRD §6) — this extension has no business on any
    // other site, and never issues a network request of its own anywhere.
