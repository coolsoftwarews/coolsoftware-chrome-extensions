...
const EXTENSION_NAME = IS_PROD ? 'X Thread Unroll & Reader' : 'X Thread Unroll & Reader (Dev)';
...
// PRD-25 §6's non-functional requirements table names the permission set
// exactly: activeTab, storage, downloads, sidePanel + host access to x.com
// and twitter.com. Nothing broader: no <all_urls>, no "tabs", no
// "scripting" (the content script is declared statically below, so it
// needs no injection permission).
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Unroll any thread on X into one clean reading view, right in your own tab — then export it as Markdown or plain text. No account, no server, no republishing.',
    icons: {
    ...
    action: {
      default_title: 'Open X Thread Unroll',
    },
    ...
    commands: {
      'open-reader-panel': {
        ...
        description: 'Open the X Thread Unroll reading panel',
      },
    },
