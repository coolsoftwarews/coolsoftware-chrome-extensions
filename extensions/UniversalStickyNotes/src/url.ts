 ...
 *
 * Ported directly from WebHighlighter's src/url.ts (PRD §4 calls for reusing
 * this logic rather than reinventing it). The same article reached from a
 * newsletter, a search result and a bookmark is three different URL strings;
 * if each got its own key, a sticky note left on one visit would simply not
 * be there on the next, which reads as data loss even though nothing was lost.
 */
