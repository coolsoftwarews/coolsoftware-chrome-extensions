...
const EXTENSION_NAME = IS_PROD ? 'Universal Table/List → CSV Scraper' : 'Table Scraper (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Click a table or repeated list on any page, preview it as rows and columns, export CSV or JSON. No account, no cloud — everything stays on your device.',
    icons: {
    ...
    action: {
      default_title: 'Universal Table/List → CSV Scraper',
      default_popup: 'popup.html',
    },
    // <all_urls> is unavoidable for a universal exporter (PRD §6): a table
    // scraper that only works on an approved list of sites isn't one.
