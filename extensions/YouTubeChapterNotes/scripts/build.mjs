...
// "… for YouTube" rather than a leading "YouTube …": same brand-safety
// reasoning YouTubeTranscription's build.mjs already documents.
const BASE_NAME = 'Chapter Notes & Timestamp Exporter for YouTube';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Save a timestamped note while you watch — with or without captions. Click to jump back, export as Markdown or CSV, no account needed.',
    icons: {
    ...
    // Chrome truncates hard at 12 characters, so this is the name, not an
    // abbreviation of it — same convention as YouTubeTranscription's manifest.
    short_name: 'ChapterNotes',
    action: {
      default_title: 'Open chapter notes panel',
      default_icon: {
        16: 'icons/icon-16.png',
        32: 'icons/icon-32.png',
        48: 'icons/icon-48.png',
    ...
    /*
     * Deliberately narrow (PRD-29 §6). This product's only two data inputs
     * are `video.currentTime` and the tab's own URL — both already available
     * to a declaratively injected content script — so `scripting` and
     * `activeTab` grant nothing that isn't already there. `tabs` is absent
     * too: chrome.tabs.query({active, currentWindow}) and
     * chrome.tabs.sendMessage both work without it once host_permissions
     * already covers the only site this extension touches.
     */
    ...
    commands: {
      'toggle-note-capture': {
        ...
        description: 'Save a timestamped note on the current video',
      },
    },
