 ...
 * instrumentation). PRD §8's own metric list — note captured, deleted,
 * edited, seek used, search used, export by format, data exported/imported —
 * each a counter in chrome.storage.local: no URLs, no note text, no video
 * titles, no timestamps beyond a day bucket. Readable and resettable from
 * the panel's Usage sheet. Ported wholesale from RedditVoiceOfCustomer's
 * metrics.ts — this shape needs no changes per product.
 */

const STORAGE_KEY = 'ycn:metrics';
