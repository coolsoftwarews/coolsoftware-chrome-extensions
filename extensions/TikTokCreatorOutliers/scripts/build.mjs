...
const EXTENSION_NAME = IS_PROD ? 'TikTok Creator Outlier Finder' : 'TikTok Creator Outliers (Dev)';
...
async function copyStaticAssets() {
  ...
  await fs.promises.copyFile(path.join(srcDir, 'popup.html'), path.join(distDir, 'popup.html'));
...
async function writeManifest(backgroundScript, contentScript, popupScript) {
  const manifest = {
    ...
    description:
      "Badge outlier videos on any TikTok profile against that creator's own median, and see what the outliers' hooks have in common. No account, no cloud.",
    icons: {
    ...
    action: {
      default_title: 'TikTok Creator Outliers',
      default_popup: 'popup.html',
    },
    // Exactly PRD-09 §6's permission table — no sidePanel, no scripting, no
    // broad tabs access. content_scripts below is static, so injection needs
    // no runtime permission beyond the host match.
  ...
  // popupScript is referenced by popup.html itself (copied verbatim), not the
  // manifest — keep the parameter so a rename of popup.ts's output surfaces
  // as a stale reference here rather than a silent 404 in the popup.
  if (popupScript !== 'popup.js') {
    throw new Error(`popup.html references popup.js but esbuild produced ${popupScript}`);
  }
}
...
function buildOptions() {
  ...
  return {
    ...
    popup: {
      ...
      format: 'iife', // classic <script src> in popup.html
      entryNames: '[name]',
    },
  };
}
