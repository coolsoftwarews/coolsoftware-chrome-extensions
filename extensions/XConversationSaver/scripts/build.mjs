...
const EXTENSION_NAME = IS_PROD ? 'X Conversation Saver' : 'X Conversation Saver (Dev)';
...
// PRD §7's non-functional requirements table names the permission set
// exactly: activeTab, storage, downloads + host access to x.com and
// twitter.com. sidePanel is added because the Library panel (PRD §4) is
// built on the Side Panel API — there is no lighter way to show it. Nothing
// broader: no <all_urls>, no "tabs", no "scripting" (the content script is
// declared statically below, so it needs no injection permission).
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Save posts and threads from X with a click — full thread text, your notes, collections, and export to Markdown, CSV or JSON. No account, no cloud.',
    icons: {
    ...
    action: {
      default_title: 'Open X Conversation Saver',
    },
    ...
    commands: {
      'open-library-panel': {
        ...
        description: 'Open the X Conversation Saver library panel',
      },
    },
