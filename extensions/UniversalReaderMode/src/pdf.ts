 ...
 *
 * Ported unchanged from WebHighlighter (which itself ported it from the
 * YouTube Transcript Export extension) — see the README's shared-modules
 * table. A headless renderer or an embedded font would cost megabytes; a
 * plain PDF 1.4 with the two standard Helvetica faces costs a few kilobytes
 * and opens in every viewer.
 *
 * Consequence of using a standard font: the encoding is WinAnsi, so glyphs
 * outside Latin-1 cannot be drawn. Those are replaced and reported back
 * through `unsupportedCharacters` so the caller can steer the user to
 * Markdown/TXT instead.
 */
 ...
 * recorded, since silently dropping characters would make a non-Latin
 * article export as a page of blanks with no explanation.
 */
