...
const EXTENSION_NAME = IS_PROD ? 'Etsy Niche Opportunity Finder' : 'Etsy Niche Finder (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      'Read the shape of an Etsy niche — seller moat, shop concentration, freshness — directly on the search page. No account, no API, everything stays on your device.',
    icons: {
    ...
    action: {
      default_title: 'Etsy Niche Opportunity Finder',
      default_popup: 'panel.html',
    },
    // Exactly the permission set in PRD-20 §6 — nothing broader. The strip and
    // badges are painted by a statically declared content script; "activeTab"
    // exists only so the popup can identify + message the active Etsy tab.
