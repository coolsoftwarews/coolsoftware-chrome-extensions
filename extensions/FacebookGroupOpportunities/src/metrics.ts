 ...
 * matched-post rate, "replied" rate, export usage) need instrumentation, but
 * the extension has no backend and asks for no identity — so every event is a
 * counter in chrome.storage.local: no group names, no post text, no
 * identifiers, nothing that leaves the device. Readable and resettable from
 * the panel footer.
 */

const STORAGE_KEY = 'fgo:metrics';
