...
const EXTENSION_NAME = IS_PROD ? 'Facebook Group Opportunity Finder' : 'Group Opportunity Finder (Dev)';
...
async function writeManifest(backgroundScript, contentScript) {
  const manifest = {
    ...
    description:
      "Flags 'looking for X' posts in the Facebook groups you're already in, using your own local rules. No account, no cloud, no automation.",
    icons: {
    ...
    action: {
      default_title: 'Open Group Opportunity Finder',
    },
    // Facebook only (PRD §6). No <all_urls>, no scripting permission — the
    // content script is statically registered for this one host.
